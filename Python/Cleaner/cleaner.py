...
class Cleaner:
    def __init__(self):
        ...
        self.arcs_dir = self.config['GENERAL']['archives_path']
    ...
    def check_dir_size_mb(input_dir):
        size = subprocess.run(['du', '-m', input_dir],
                              stdout=subprocess.PIPE).stdout.decode('utf-8')
        return int(size.split()[0])

    @staticmethod
    def get_oldest_file(input_dir, extension, reverse=False):
        files = {os.path.getmtime(os.path.join(input_dir, file)):
    ...
    def mng_cache(self):
        user_cache, root_cache, var_cache = \
            os.path.expanduser('~/.cache'), '/root/.cache', '/var/cache'
        user_cache_excp = self.config['CACHE']['user_exceptions']
        root_cache_excp = self.config['CACHE']['root_exceptions']
        var_cache_excp = self.config['CACHE']['var_exceptions']

        tmp_dir = f'/tmp/cache_management_{self.today}'
        tmp_user, tmp_root, tmp_var = f'{tmp_dir}/user_cache', \
            f'{tmp_dir}/root_cache', f'{tmp_dir}/var_cache'
        self.create_folder(' '.join([tmp_user, tmp_root, tmp_var]))

        if os.listdir(user_cache) != 0:
            [self.move_file_or_folder(os.path.join(user_cache, file_dir),
                                      tmp_user)
                for file_dir in os.listdir(user_cache)
                if file_dir not in user_cache_excp]

        if os.listdir(root_cache) != 0:
            [self.move_file_or_folder(os.path.join(root_cache, file_dir),
                                      tmp_root)
                for file_dir in os.listdir(root_cache)
                if file_dir not in root_cache_excp]

        if os.listdir(var_cache) != 0:
            [self.move_file_or_folder(os.path.join(var_cache, file_dir),
                                      tmp_var)
        ...
        archives_max_mb = int(self.config['CACHE']['archives_max_mb'])
    ...
    def mng_history(self):
        user_hist_list = self.config['HISTORY']['user_paths']
        root_hist_list = self.config['HISTORY']['root_paths']
        ...
        [self.move_file_or_folder(file, tmp_user) for file in user_hist_list
            if os.path.isfile(file)]

        [self.move_file_or_folder(file, tmp_root) for file in root_hist_list
            if os.path.isfile(file)]
    ...
    def mng_symlinks(self):
        ...
        broken_slinks_list = subprocess.run(cmd, stdout=subprocess.PIPE,
        ...
        for link in broken_slinks_list:
            if not os.path.islink(link):
                continue
            if link.startswith('/run') or link.startswith('/proc') \
    ...
    def mng_worthless_files(self):
        files_list = self.config['WORTHLESS']['files']
        folders_list = self.config['WORTHLESS']['folders']
    ...
    def mng_pac_orphans(self):
        cmd = self.config['PACMANAGER']['command']
