...
class AlarmsListener:
    def __init__(self):
        ...
        self.notif_sound = self.config['ALARMS']['notif_sound']
    ...
    def check_next_alarm(self):
        ...
        self.next_alarm_date_strp = datetime.strptime(self.next_alarm_date,
                                                      '%Y-%m-%d %H:%M')
    ...
    def send_notification(self):
        cmd = f'paplay {self.notif_sound} & notify-send "{self.notif}"' \
            if not self.notif_is_mute else f'notify-send "{self.notif}"'
