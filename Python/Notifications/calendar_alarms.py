...
class CalendarAlarms:
    def __init__(self):
        ...
        self.cals_path = self.config['CALENDARS']['calendars_path']
    ...
    def main(self):
        ...
        for self.cal_file in self.cal_files_list:
            ...
            is_rec_and_past = ('personal' not in self.cal_file and
                               self.cal_file_uid not in self.alarms_uids_list)
    ...
    def get_alarms_file_data(self):
        ...
        self.cal_alarms_last_update_strp = \
            datetime.strptime(self.cal_alarms_last_update, '%Y-%m-%d %H:%M:%S')
    ...
    def get_cal_files_list(self):
        self.cal_files_list = [os.path.join(root_dirs_files[0], file)
    ...
    def check_if_new(self):
        ...
        cal_file_modif_date = datetime.fromtimestamp(cal_file_modif_date) \
            .strftime('%Y-%m-%d %H:%M:%S')

        cal_file_modif_date_strp = datetime.strptime(cal_file_modif_date,
                                                     '%Y-%m-%d %H:%M:%S')
    ...
    def delete_event_alarms(self):
        ...
        self.alarms_list = [alarm for alarm in self.alarms_list
    ...
    def get_alarm_date_and_msg(self, alarm):
        ...
        if alarm_time == '0':
            ...
            if not alarm_time_positive:
                self.new_alarm_date = cal_event_date \
                    - timedelta(seconds=alarm_time_secs)
                # singular
                if alarm_time == '1':
        ...
        if self.recurrent_event:
            while self.now_strp > self.new_alarm_date:
                if self.new_alarm_recurrency == 'YEARLY':
                    ...
                    self.new_alarm_date = self.new_alarm_date \
                        + timedelta(days=7)
    ...
    def create_alarm(self):
        ...
        new_alarm_date_strp = \
            datetime.strptime(str(self.new_alarm_date), '%Y-%m-%d %H:%M:%S')
        for alarm in self.alarms_list:
            alarm_date = alarm['date']
            alarm_date_strp = \
                datetime.strptime(str(alarm_date), '%Y-%m-%d %H:%M')
    ...
    def check_if_cal_file_deleted(self):
        ...
        self.alarms_list = [alarm for alarm in self.alarms_list
                            if alarm['uid'] in self.cals_uids_list
                            or alarm['uid'] == '#N/A']
