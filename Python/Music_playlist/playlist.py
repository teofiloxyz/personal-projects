...
class Playlist:
    ...
    def show(self, mode: str) -> None:
        ...
        df = self.query_db.get_all_music(self.playlist, selection)
        if len(df) == 0:
    ...
    def add(
        ...
        self.edit_db.add_music(self.playlist, entry)

    def remove(self, entry: (tuple | None) = None) -> None:
        if entry is None:
            title = self.show_options()
            if title == "q":
                return
            entry = tuple(
                self.query_db.get_selection_with_title(
        ...
        self.edit_db.remove_music(self.playlist, title)
    ...
    def recover_arc(self) -> None:
        ...
        if title == "q":
            return
        entry = self.query_db.get_selection_with_title(
            self.playlist, "*", title
        )[1:]
        self.remove(entry)
        self.playlist = "playlist"
    ...
    def edit_entry(self) -> None:
        ...
        if option.lower() == "t":
            new_title = self.pick_title(self.playlist, title)
            if self.playlist == "playlist":
                for music_file in os.listdir(self.music_path):
            ...
            self.edit_db.update_playlist(
                self.playlist, "title", new_title, title
            )
            print(f'Title changed from "{title}" to "{new_title}"')
        elif option.lower() == "g":
            genre = self.query_db.get_selection_with_title(
    ...
    def show_options(
        ...
        while True:
            ...
            if option == "q":
                ...
                selection = self.query_db.get_all_titles(self.playlist)
            elif option == "ls":
                ...
                try:
                    selection = self.query_db.get_title_with_id(
    ...
    def search_by_title(self, query: (str | None) = None) -> (tuple | str):
        ...
        result = self.query_db.get_title_with_search(
    ...
    def search_by_genre(self) -> (tuple | str):
        ...
        if len(result) == 0:
            ...
            print(f"Found {len(result)} tracks with genre: {selected_genres}")
        return result
    ...
    def pick_title(playlist: str, title: str) -> str:
        ...
        titles = Query().get_all_titles(playlist)
