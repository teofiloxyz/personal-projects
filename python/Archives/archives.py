...
class Archives:
    ...
    def check_option(self):
        self.opst_dict = {'extract': self.extract,
        ...
        if self.option in self.opst_dict.keys():
            ...
            self.error("First argument needs to be 'extract', "
                       "'compress' or 'add'")
    ...
    def check_input(self):
        ...
        self.entire_folder = True if self.option != 'add' and self.input in \
            ('', 'all', 'a', 'al') else False
    ...
    def check_input2(self):
        ...
        if extension not in self.arc_types.keys():
            self.error(f"'{self.input2}' is not a recognized archive "
                       "format...")
    ...
    def extract(self):
        ...
        def get_arc_name(self):
            ...
            # As próximas têm que ser assim devido aos casos ".tar.gz"
            # Começa em [1:] devido a potenciais hidden files
            # Melhorar esta limitação
            if self.basename[1:].count('.') == 1:
                self.name, self.extension = os.path.splitext(self.basename)
            elif self.basename[1:].count('.') == 2:
                name, self.extension = os.path.splitext(self.basename)
                self.name, ex2 = os.path.splitext(name)
                if ex2 == '.tar':
                    self.extension = ex2 + self.extension
            else:
                self.error('Input has a problem with extension '
                           '(too many dots on the file name?)')
        ...
        def create_extracted_dir(self):
            ...
            self.extracted_dir = os.path.join(input_dir, 'Extracted_'
                                              + self.name)
            ...
            while os.path.isdir(self.extracted_dir):
                ...
                if n == '':
                    self.extracted_dir = \
                        os.path.join(input_dir, '2Extracted_' + self.name)
                else:
                    self.extracted_dir = \
                        os.path.join(input_dir, str(int(n) + 1)
                                     + 'Extracted_' + self.name)

            os.mkdir(self.extracted_dir)

        def move_to_extracted_dir(self):
            self.input_on_extracted_dir = \
                os.path.join(self.extracted_dir, self.basename)
        ...
        def extract_archive(self):
            ...
            cmd = self.arc_types[self.extension] + ' ' \
                + self.input_on_extracted_dir
        ...
        def open_ranger_on_extracted_dir(self):
            if not input(':: Do you want to open ranger on the extraction '
                         'folder? [Y/n] ') in ('', 'y', 'Y'):
                return
        ...
        if self.entire_folder:
            ...
            arcs_list = [os.path.join(os.getcwd(), file)
    ...
    def compress(self):
        ...
        def name_output(self):
            self.output = \
                oupt.files(question='Enter the output full path, or '
                           'just the name for same input dir, or leave '
                           'empty for same as input\n: ',
                           file_input=self.input, output_name_change=True)
        ...
        def get_compression_method(self):
            ...
            if os.path.isfile(self.input):
                ...
                if os.path.basename(self.output).split('.')[0] == '':
                    self.output += \
                        os.path.dirname(self.input_folder).split('/')[-1]
                methods = \
                    {'1': ('tar.xz(max)',
                           'tar.xz',
        ...
        def compress_input(self):
            if self.method[1] == '7z':
                if input(':: Want encryption? You should! '
                         '[Y/n] ') in ('', 'y', 'Y'):
                    cmd = self.method[2][0]
                else:
                    cmd = self.method[2][1]
    ...
    def add_to_archive(self):
        ...
        def navigate_in_extracted(self):
            ...
            while len(current_dir_folders) != 0:
                ...
                next_dir = input('Pick the number of folder to open, or leave '
        ...
        def copy_input_to_extracted(self):
            ...
            arc_dst = os.path.join(arc_dst_dir,
                                   os.path.basename(self.input_to_add))
